...
RSpec.feature 'Order management', type: :feature do
  ...
  context 'User authenticated' do
    ...
    scenario 'Order total visible' do
      ...
      fill_in 'Quantity', with: '2'
      click_button 'Save'
      expect(page).to have_text("$#{(@line_item.price_at_purchase * 2)}")
    end
    ...
    scenario 'Edit order' do
      ...
      expect(page).to have_text("$#{(@line_item.price_at_purchase * 3)}")
    end
    ...
    scenario 'Delete item' do
      ...
      visit '/orders'
      click_link 'Destroy'
      expect(page).to have_text('Order was successfully deleted')
    end
